...
async function ensureCourseMentions(courseId: bigint, mentions: string[], res: Response): Promise<boolean> {
  if (mentions.length === 0) return true;

  const [members, teachers, assistants] = await Promise.all([
  ...
  if (invalid.length > 0) {
    validationFailed(res, `mentions must be course members: ${invalid.join(", ")}`);
    return false;
  }
...
// ──────────────────────────────────────────────────────────────
courseChatRouter.get("/courses/:courseId/messages", requireAuth, async (req: Request, res: Response) => {
...
// ──────────────────────────────────────────────────────────────
courseChatRouter.post("/courses/:courseId/messages", requireAuth, async (req: Request, res: Response) => {
  const courseId = parseBigIntParam(req.params.courseId, "courseId", res);
  if (courseId === null) return;
  ...
  for (const file of filesInput) {
    ...
    if (!isObjectKeyInScope(file.objectKey, "course", courseId.toString())) {
      return validationFailed(res, "file objectKey does not match course scope");
    }
  }
  ...
  if (!(await ensureCourseMentions(courseId, mentions, res))) return;
  ...
  await pushSocketEvent(`course:${courseId.toString()}`, outboundEvent);
...
// ──────────────────────────────────────────────────────────────
courseChatRouter.post("/courses/:courseId/announcements", requireAuth, async (req: Request, res: Response) => {
  const courseId = parseBigIntParam(req.params.courseId, "courseId", res);
  if (courseId === null) return;
...
// ──────────────────────────────────────────────────────────────
courseChatRouter.get("/courses/:courseId/messages/search", requireAuth, async (req: Request, res: Response) => {
  const courseId = parseBigIntParam(req.params.courseId, "courseId", res);
  if (courseId === null) return;
...
// ──────────────────────────────────────────────────────────────
courseChatRouter.patch("/courses/:courseId/messages/:messageId", requireAuth, async (req: Request, res: Response) => {
  const courseId = parseBigIntParam(req.params.courseId, "courseId", res);
  if (courseId === null) return;
  ...
  const event = createEventEnvelope("course.message.updated", {
    courseId: courseId.toString(),
...
// ──────────────────────────────────────────────────────────────
courseChatRouter.delete("/courses/:courseId/messages/:messageId", requireAuth, async (req: Request, res: Response) => {
  const courseId = parseBigIntParam(req.params.courseId, "courseId", res);
  if (courseId === null) return;
  const messageId = parseBigIntParam(req.params.messageId, "messageId", res);
  if (messageId === null) return;
  ...
  const event = createEventEnvelope("course.message.deleted", {
    courseId: courseId.toString(),
    messageId: messageId.toString(),
