...
补充说明：
- 每条 `Submission` 现在包含 `files` 数组。
- `files` 项结构为：`{ id, name, size, mimeType, uploadedAt, downloadPath }`
- 前端可继续请求 `downloadPath` 获取真实下载地址。
  ...
  "data": [
    {
      ...
      "attemptNo": 3,
      "status": "submitted",
      "summary": "提交需求说明书与原型地址",
      "submittedBy": "2026010001",
      "submittedAt": "2026-06-18T10:15:00+08:00",
      "createdBy": "2026010001",
      "createdAt": "2026-06-18T10:15:00+08:00",
      "updatedAt": "2026-06-18T10:15:00+08:00",
      "files": [
        {
          "id": "301",
          "name": "需求说明模板.docx",
          ...
          "uploadedAt": "2026-06-18T10:14:31+08:00",
          "downloadPath": "/api/v1/submission-files/301/download-url"
        }
      ]
    }
  ]
...
- Method: GET
- Path: /api/v1/submission-files/{fileId}/download-url
- Headers:
  - Authorization: Bearer `<token>`
- Response:
  - id: string
...
说明：
- 仅有该提交访问权限的学生、老师、助教、教务可获取下载地址。
- `downloadUrl` 为短时有效的预签名地址，供前端直接发起下载。
...
| 阶段提交 | GET | /api/v1/stages/{stageId}/groups/{groupId}/submissions | 获取该小组在某阶段的提交记录（含附件元数据） |
| 提交附件下载地址 | GET | /api/v1/submission-files/{fileId}/download-url | 获取提交附件预签名下载地址 |
