...
如果 auth 负责同学希望恢复某个旧决策，应先同步修改 `docs/api/auth/认证表结构设计.md`、`apps/api/prisma/schema.prisma` 和相关 API 契约，避免出现两套事实来源。
...
- 现阶段不建 `refresh_tokens` 表，避免和 [认证表结构设计.md](认证表结构设计.md) 冲突。
